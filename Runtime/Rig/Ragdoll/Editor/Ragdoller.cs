...
namespace KadenZombie8.BIMOS.Ragdoll.Editor
{
	...
	sealed class Ragdoller
	{
		...
		readonly PhysicsRagdoll _ragdoll;
		...
		public Ragdoller(Transform player, Vector3 playerDirection)
		{
			...
			// find Animator
			_animator = FindAnimator(player);
			if (_animator == null)
				return;
			_rootNode = _animator.transform;
			_ragdoll = _rootNode.gameObject.AddComponent<PhysicsRagdoll>();

			// specify all parts of ragdoll
			var physicsAnimator = GameObject.Instantiate(_animator, _rootNode.parent);
			var virtualHips = _animator.GetBoneTransform(HumanBodyBones.Hips);
            _pelvis = new RagdollPartBox(physicsAnimator, _animator, HumanBodyBones.Hips);
			_leftHip = new RagdollPartCapsule(physicsAnimator, _animator, HumanBodyBones.LeftUpperLeg);
			_leftKnee = new RagdollPartCapsule(physicsAnimator, _animator, HumanBodyBones.LeftLowerLeg);
			_rightHip = new RagdollPartCapsule(physicsAnimator, _animator, HumanBodyBones.RightUpperLeg);
			_rightKnee = new RagdollPartCapsule(physicsAnimator, _animator, HumanBodyBones.RightLowerLeg);
			_leftArm = new RagdollPartCapsule(physicsAnimator, _animator, HumanBodyBones.LeftUpperArm);
			_leftElbow = new RagdollPartCapsule(physicsAnimator, _animator, HumanBodyBones.LeftLowerArm);
			_rightArm = new RagdollPartCapsule(physicsAnimator, _animator, HumanBodyBones.RightUpperArm);
			_rightElbow = new RagdollPartCapsule(physicsAnimator, _animator, HumanBodyBones.RightLowerArm);
			_chest = new RagdollPartBox(physicsAnimator, _animator, HumanBodyBones.Chest);
			_head = new RagdollPartSphere(physicsAnimator, _animator, HumanBodyBones.Head);

			_leftFoot = new RagdollPartBox(physicsAnimator, _animator, HumanBodyBones.LeftFoot);
			_rightFoot = new RagdollPartBox(physicsAnimator, _animator, HumanBodyBones.RightFoot);
			_leftHand = new RagdollPartBox(physicsAnimator, _animator, HumanBodyBones.LeftHand);
			_rightHand = new RagdollPartBox(physicsAnimator, _animator, HumanBodyBones.RightHand);
            
            if (_chest.transform == null)
				_chest = new RagdollPartBox(physicsAnimator, _animator, HumanBodyBones.Spine);				
            ...
            _pelvis.transform.parent = _animator.GetBoneTransform(HumanBodyBones.Hips).parent;
            _pelvis.transform.name += " (Ragdoll)";

            List<SkinnedMeshRenderer> renderers = _animator.GetComponentsInChildren<SkinnedMeshRenderer>().ToList();
            foreach (var renderer in renderers) {
                if (renderer.rootBone == virtualHips)
                    renderer.rootBone = _pelvis.transform;
            }
            GameObject.DestroyImmediate(physicsAnimator.gameObject);
